...
\begin{table}[!h] % begins table environment. The argument !h forces the position of the table to be "here". 
    \begin{center} %begins the center environment.
      \begin{tabular}{l|l|l}  %Begins tabular environment. Each column is separated b y an "|" and the text is aligned left in each column.
        ...
        Number & Symbol & Description\\  % each column is separated using '&'. The \\ is used to create new row
        ...
        \hline % horizontal line at bottom of table
     \end{tabular} % ends tabular environment
     \caption{Symbols and Definitions} % creates caption for a table.
...
\begin{figure}[!h] % begins the figure environment and forces the figure to appear "here".
    \begin{center} % centers the following 
        \includegraphics[width=10cm]{horse.png}  % imports graphic as a specified size 
